...
dThin <- nimbleFunction(
  run = function(x = double(2), y.true = double(2), lambda = double(2), obs.i = double(1),
    ...
    M <- nimDim(y.true)[1]
    J <- nimDim(y.true)[2]
    #track which individuals and traps remain available
    i.available <- rep(1,M)
    j.available <- rep(1,J)
    y.is.one <- y.true==1 #which elements of y.true are 1?
    #identify indices of latent capture events
    ...
    for(o in 1:(length(order)-1)){ #we do not need the final logProb which is always 0
      idx <- order.idx[o] #observation with capture order o
      focal.lambda <- lambda[obs.i[idx],obs.j[idx]] #focal individual/trap must still be available
      #count remaining latent capture events
      n.other.lambdas <- 0
      for(l in 1:n.latent){
        i <- latent.i[l]
        j <- latent.j[l]
        if(i.available[i]==1){ #individual has not already been captured
          if(j.available[j]==1){ #trap has not already been filled
      ...
      if(n.other.lambdas>0){
        ...
        for(l in 1:n.latent){
          i <- latent.i[l]
          j <- latent.j[l]
          if(i.available[i]==1){ #ndividual has not already been captured
            if(j.available[j]==1){ #trap has not already been filled
      ...
      #mark individual and trap unavailable
