...
sim.SCR.multiCatch.Dcov <- function(D.beta0=NA,D.beta1=NA,D.cov=NA,InSS=NA,xlim=NA,ylim=NA,res=NA,
                               p0=NA,sigma=NA,X=NA,K=NA){
  #get expected N
  cellArea <- res^2
  ...
  points(X,pch=4)
  return(list(y=y,n.cap=n.cap,X=X,K2D=K2D,
