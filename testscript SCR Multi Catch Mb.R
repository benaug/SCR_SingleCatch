...
library(nimble)
library(coda)
source("sim.SCR.multicatch.Mb.R") # data simulator
source("NimbleModel SCR Multi Catch Mb.R") #nimble model file
source("NimbleFunctions SCR Multi Catch Mb.R") #nimble functions and custom updates
...
Niminits <- list(lambda.N=N.init,p0.p=runif(1,0.1,0.9),p0.c=runif(1,0.1,0.9),
...
parameters <- c('p0.p','p0.c','sigma','lambda.N','N')
nt <- 1 #thinning rate
start.time <- Sys.time()
