    ...
    for(up in 1:y.ups){ #update one or more times per iteration
      ...
      for(c in 1:n.obs.cells.all){
        ...
        if(!skip){ #skip if no one to turn off
          ...
          #update y.state
          for(k2 in 1:K){
            y.state.cand[select.cand,obs.j[c],k2] <- 0
          }
          hasSamps <- sum(y.true.cand[select.cand,obs.j[c],])>0
          if(hasSamps){
            firstcap <- which(y.true.cand[select.cand,obs.j[c],]>0)[1]
            if(firstcap<K){
              for(k2 in (firstcap+1):K){
                y.state.cand[select.cand,obs.j[c],k2] <- 1
              }
            }
          }

          #update observation model likelihood
          for(k2 in 1:K){#all k may change
            if(K2D[obs.j[c],k2]>0){
              if(y.state.cand[select.cand,obs.j[c],k2]==0){
                ll.y.cand[select.cand,obs.j[c],k2] <- dbinom(y.true.cand[select.cand,obs.j[c],k2],1,pd.p[select.cand,obs.j[c]],log=TRUE)
              }else{
                ll.y.cand[select.cand,obs.j[c],k2] <- dbinom(y.true.cand[select.cand,obs.j[c],k2],1,pd.c[select.cand,obs.j[c]],log=TRUE)
              }
            }
          }

          # update thinning likelihood - need to consider all occasions
          for(k2 in 1:K){
            ll.y.obs.cand[k2] <- dThin(x=y.obs[1:n.cap,1:J,k2],y.true=y.true.cand[1:M,1:J,k2],y.state=y.state.cand[1:M,1:J,k2],
                                      lambda.p=lambda.p[1:M,1:J],lambda.c=lambda.c[1:M,1:J],
                                      obs.i=obs.i2D[1:n.obs.cells[k2],k2],
                                      obs.j=obs.j2D[1:n.obs.cells[k2],k2],
                                      order=order2D[1:n.obs.cells[k2],k2],
          ...
          logProb.curr <- sum(ll.y.obs) +  sum(ll.y[select.cand,obs.j[c],])
          logProb.cand <- sum(ll.y.obs.cand) +  sum(ll.y.cand[select.cand,obs.j[c],])
          ...
          if(accept){
            ...
            y.state[select.cand,obs.j[c],] <- y.state.cand[select.cand,obs.j[c],]
            ll.y[select.cand,obs.j[c],]  <- ll.y.cand[select.cand,obs.j[c],]
            ll.y.obs <- ll.y.obs.cand
          }else{
            y.true.cand[select.cand,obs.j[c],obs.k[c]] <- y.true[select.cand,obs.j[c],obs.k[c]]
            y.state.cand[select.cand,obs.j[c],] <- y.state[select.cand,obs.j[c],]
            ll.y.cand[select.cand,obs.j[c],]  <- ll.y[select.cand,obs.j[c],]
            ll.y.obs.cand <- ll.y.obs
          }
        }
      }
      ...
      for(k in 1:K){ #loop over occasions
        for(i in 1:n.obs.cells[k]){ #loop over captured individuals
          ...
          if(!skip){ #skip if no one to turn off
            ...
            #update y.state
            for(k2 in 1:K){
              y.state.cand[this.i,select.cand,k2] <- 0
            }
            hasSamps <- sum(y.true.cand[this.i,select.cand,])>0
            if(hasSamps){
              firstcap <- which(y.true.cand[this.i,select.cand,]>0)[1]
              if(firstcap<K){
                for(k2 in (firstcap+1):K){
                  y.state.cand[this.i,select.cand,k2] <- 1
                }
              }
            }
            #update observation model likelihood
            for(k2 in 1:K){#all k may change
              if(K2D[this.i,k2]>0){
                if(y.state.cand[this.i,select.cand,k2]==0){
                  ll.y.cand[this.i,select.cand,k2] <- dbinom(y.true.cand[this.i,select.cand,k2],1,pd.p[this.i,select.cand],log=TRUE)
                }else{
                  ll.y.cand[this.i,select.cand,k2] <- dbinom(y.true.cand[this.i,select.cand,k2],1,pd.c[this.i,select.cand],log=TRUE)
                }
              }
            }
            ...
            logProb.curr <- sum(ll.y.obs) + sum(ll.y[this.i,select.cand,])
            logProb.cand <- sum(ll.y.obs.cand) + sum(ll.y.cand[this.i,select.cand,])
            log_MH_ratio <-  (logProb.cand + log(select.probs.back[select.cand])) - (logProb.curr + log(select.probs.for[select.cand]))
            
            accept <- decide(log_MH_ratio)
            if(accept){
              y.true[this.i,select.cand,k] <- y.true.cand[this.i,select.cand,k]
              y.state[this.i,select.cand,] <- y.state.cand[this.i,select.cand,]
              ll.y[this.i,select.cand,]  <- ll.y.cand[this.i,select.cand,]
              ll.y.obs <- ll.y.obs.cand
            }else{
              y.true.cand[this.i,select.cand,k] <- y.true[this.i,select.cand,k]
              y.state.cand[this.i,select.cand,] <- y.state[this.i,select.cand,]
              ll.y.cand[this.i,select.cand,]  <- ll.y[this.i,select.cand,]
              ll.y.obs.cand <- ll.y.obs
            }
          }
        }
