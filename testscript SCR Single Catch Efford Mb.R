...
#First, it is slower than M0 with a single p0. 
#Second, the observation model parameter estimates are extremely
#imprecise when you do not know the capture order with certainty.
#Finally, I have not tested this, yet.
#I reused code from the Random-Thin-MVhyper repository
#that considers multivariate hypergeometric subsampling
#that occurs when you keep X samples per trap-occasion (common for hair snares).
#I have tested the Mb part of the code there.
...
str(data$y.state) #observed capture states (first vs subsequent)
...
#augment y.state
y.state.init <- array(0,dim=c(M,J,K))
y.state.init[1:data$n.cap,,] <- data$y.state
...
Niminits <- list(lambda.N=N.init,p0.p=runif(1,0.1,0.5),p0.c=runif(1,0.3,0.9),sigma=runif(1,0.5,1),
                 s=s.init,y.true=y.true.init,y.state=y.state.init,z=z.init,N=N.init,
...
Nimdata <- list(X=data$X,y.obs=data$y.obs)
