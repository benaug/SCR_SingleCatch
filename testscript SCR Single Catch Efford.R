library(nimble)
library(coda)
source("sim.SCR.singleCatch.R") # data simulator
source("NimbleModel SCR Single Catch Efford.R") #nimble model file
source("NimbleFunctions SCR Single Catch Efford.R") #nimble functions and custom updates
...
parameters <- c('p0','sigma','lambda.N','N','y.true.sum')
nt <- 1 #thinning rate
start.time <- Sys.time()
