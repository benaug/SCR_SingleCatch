...
library(coda)
source("sim.SCR.multicatch.Dcov.R") # data simulator
source("NimbleModel SCR Multi Catch Dcov.R") #nimble model file
source("NimbleFunctions SCR Multi Catch Dcov.R") #nimble functions and custom updates
...
Nimdata <- list(X=data$X,y=y,dummy.data=dummy.data,cells=cells,InSS=data$InSS)

# set parameters to monitor
parameters <- c('p0','sigma','D0','D.beta1','lambda.N','N')
